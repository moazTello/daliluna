...
const Row = ({ content, hr1, hr2, hr3, trash, fatherId }) => {
  ...
  const { deleteClassifiedDepartment, loadingDeleteDepartment } = useDeleteClassifiedDepartment();
  const {deleteYellowpage,deleteYellowloading} = useDeleteYellowPages();
    const navigate = useNavigate();
    const deleteclass = async () => {
      hr1 === "/classifieds/classifiedsdepartments" && await deleteClassifieds(content.id);
      hr1 === "addpost" && await deleteClassifiedDepartment(fatherId,content.id);
      hr1 === "/yellowpages/yellowpagesdepartments" && await deleteYellowpage(content.id);
    }
    const navigation1 = () => {
        hr1 === "/classifieds/classifiedsdepartments" ? navigate(`/classifieds/${content.id}/classifiedsdepartments`) : 
        hr1 === "/yellowpages/yellowpagesdepartments" ? navigate(`/yellowpages/${content.id}/yellowpagesdepartments`) :
        "";
    }
    const navigation2 = () => {
      hr1 === "/classifieds/classifiedsdepartments" ? navigate(`/classifieds/editclassified/${content.id}`) : "";
      hr1 === "addpost" ? navigate(`/classifieds/${fatherId}/classifiedsdepartments/${content.id}`) : "";
      hr1 === "/yellowpages/yellowpagesdepartments" ? navigate(`/yellowpages/edityellowpage/${content.id}`) : "";
      hr1 === "/elements" ? navigate(`/yellowpages/${fatherId}/yellowpagesdepartments/${content.id}`) : "";
    }
    const navigation3 = () => {
      hr1 === "addpost" ? navigate(`/classifieds/${fatherId}/classifiedsdepartments/${content.id}`) : "";
    }
      ...
      <tr>
        ...
        <th>
          ...
          >
            ...
            {!loading && !loadingDeleteDepartment && !deleteYellowloading && <FaTrashAlt size={20} />}
              {(loading || loadingDeleteDepartment || deleteYellowloading) && <p className='loading loading-spinner bg-white'></p>}
            </span>
          </button>

          <button onClick={navigation2}
          ...
          {hr1 !== "none" && <button onClick={navigation1}
          ...
          >
            <span className="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
            {hr1 === "/classifieds/classifiedsdepartments" || hr1 === "/yellowpages/yellowpagesdepartments" ? "departments" : hr1}
            </span>
          </button>}
          {hr2 !== "none" && <button
            className="relative m-1  align-middle select-none font-sans font-medium text-center uppercase transition-all
