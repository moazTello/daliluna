...
import useEditYellowPageDepartment from "../../../hooks/YellowDepartments/useEditYellowPageDepartments";
...
const EditYellowPagesDepartment = () => {
  ...
  const {
    ...
    setEditedIcon,
    editedIcon,
  } = useEditYellowPageDepartment();
  const navigate = useNavigate();
  useEffect(() => {
    const getEdit = async () => {
      await getEditYellowPageDepartment(yellowpageId, yellowdepartmentId);
    };
    getEdit();
  }, []);
  const handleSubmit = async (e) => {
    e.preventDefault();
    await editYellowPageDepartment(
    ...
    <div className="flex flex-col items-center justify-center w-11/12 mx-auto">
      <div className="w-full p-6 rounded-lg shadow-md bg-white bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-0 ">
        <h1 className="text-base font-semibold text-start mt-5 text-gray-200">
          Edit Yellow Page Department <span className="text-red-600"> NOT WORKING CAUSE OF API INDEX</span>
        </h1>
